...
public class BoardRepositoryTests {
    ...
    public void testUpdate(){
        ...
        boardRepository.save(board); // .save 메서드는 없으면 insert, 있으면 update함
    ...
    public void testDelete(){
        ...
        //             .deleteById(bno) -> delect from board where bno = bno
    ...
    public  void setsPaging(){
        ...
        //Hibernate:
        //    select
        //        count(b1_0.bno)   board전체 리스트 수를 알아옴
        //    from
        //        board b1_0

        Page<Board> result = boardRepository.findAll(pageable);
        // 1장에 종에이에 Board객체를 가지고 있는 결과는 result에 담긴다.
        // Page클래스는 다음페이지 존재여부, 이전페이지 존재여부, 전테 데이터 개수 등등  계산을 한다.
        ...
        // forEach는 인텍스를 사용하지 않고 앞에서부터 객체를 리턴함
        //                board -> log.info(board)
        //                   람다식 1개의 명령어가 있을 때 좋음
    ...
    public void testSearch1(){
        ...
        Page<Board> result = boardRepository.search1(pageable); // 페이징기법을 사용해서 title = 1값을 찾아 오나 ?
    ...
    public void testSearchAll(){
        // 프론트에서 t가 선택 되면 title, c가 선택되면 content, w가 선택되면 writer가 조건으로 제시됨
