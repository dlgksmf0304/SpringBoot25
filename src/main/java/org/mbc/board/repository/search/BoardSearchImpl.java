...
public class BoardSearchImpl extends QuerydslRepositorySupport implements BoardSearch {
    ...
    public Page<Board> search1(Pageable pageable) {
        // 쿼리 dsl로 다중 검색용 코드 추가
        // 쿼리 dsl의 목적은 타입 기반으로 코드를 이용함 -> Q도메인
        ...
        query.where(booleanBuilder); // ( where title like 11 0r content like 11)
        query.where(board.bno.gt(0L)); // pk를 이용해서 빠른 검색 where이 추가되면 and 조건
        // where (title like 11 0r content like 11) and bno > 0
        ...
        long count = query.fetchCount(); // 검색후에 게시물 수 파악 용
    ...
    public Page<Board> searchAll(String[] types, String keyword, Pageable pageable) {
        ...
        // 프론트에서 검색폼에 keyword가 비었을경우도 있고 있을경우도 있다.
        if( (types != null && types.length > 0) && keyword != null ){
            // 제목, 내용, 이름 값이 있고 검색어가 있으면 !

            BooleanBuilder booleanBuilder = new BooleanBuilder(); // 선 실행용()
        ...
        // where (tite or content or writer) and bno > 0L
